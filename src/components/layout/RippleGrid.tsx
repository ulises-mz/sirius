...
type Props = {
  ...
  /** Tipo de patrón a mostrar: 'grid' (por defecto), 'radial' (anillos concéntricos), 'dots' (matriz de puntos). */
  patternType?: 'grid' | 'radial' | 'dots';
};
...
uniform float patternMode; // 0.0 = grid, 1.0 = radial, 2.0 = dots
varying vec2 vUv;
...
void main() {
  ...
  if (patternMode < 0.5) {
    ...
    float ringFreq = gridSize * 0.6; // se relaciona con gridSize
    float ring = sin(dist * ringFreq - iTime * 1.2);
    float ringAbs = abs(ring);
    float smoothRing = smoothstep(0.0, 0.4, ringAbs);
    color += exp(-gridThickness * smoothRing);
    if (glowIntensity > 0.0) {
      color += glowIntensity * exp(-gridThickness * 0.5 * smoothRing);
    }
  } else {
    // DOTS (matriz de puntos)
    vec2 scaled = rippleUv * gridSize;
    vec2 cell = floor(scaled);
    vec2 cellCenter = cell + 0.5;
    vec2 diff = scaled - cellCenter;
    float d = length(diff);
    float radius = 0.35; // tamaño del punto
    float dot = smoothstep(radius, radius - 0.15, d); // borde suavizado
    // Intensidad modulada por tiempo para animación sutil
    float pulse = 0.7 + 0.3 * sin(iTime + cell.x * 0.3 + cell.y * 0.3);
    color += pulse * (1.0 - dot);
    if (glowIntensity > 0.0) {
      color += glowIntensity * (1.0 - dot) * 0.5;
    }
  }
  ...
  const patternModeMap: Record<string, number> = { grid: 0, radial: 1, dots: 2 };
  uniforms.patternMode.value = patternModeMap[patternType] ?? 0;
  ...
  useEffect(() => {
    ...
    const patternModeMap: Record<string, number> = { grid: 0, radial: 1, dots: 2 };
    uniformsRef.current.patternMode.value = patternModeMap[patternType] ?? 0;
  }, [
    enableRainbow,
    gridColor,
    rippleIntensity,
